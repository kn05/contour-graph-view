...
export function buildContour(points: readonly Point[]): Point[] {
  if (points.length < 3) return [];
  const pairs = points.map((point): [number, number] => [point.x, point.y]);
  const hull = concaveman(pairs, CONTOUR_CONCAVITY, 0);
  const polygon: Point[] = [];
...
function drawCurve(ctx: CanvasRenderingContext2D, points: readonly Point[]): void {
  if (points.length < 3) return;
